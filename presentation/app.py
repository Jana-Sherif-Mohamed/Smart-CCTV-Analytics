        ...
        <div class="sentinel-header">
            <div>
                ...
                <p class="sentinel-subtitle">
                    Real-time AI object detection, tracking &amp; security event analytics
                </p>
            </div>
...
def resolve_video_source():
    ...
    st.sidebar.markdown(
        '<p class="section-label">Video Source</p>',
        unsafe_allow_html=True
    )

    source_type = st.sidebar.radio(
        "Input source",
        ["Select Sample Video", "Upload Video File"],
    ...
    if source_type == "Select Sample Video":
        ...
        # Load available videos from the sample directory
        video_dir = "videos"
        ...
        if video_files:
            ...
            st.sidebar.warning(
                "No sample videos found in the `videos/` directory."
            )
...
def resolve_model_settings(config: dict) -> dict:
    ...
    st.sidebar.markdown(
        '<p class="section-label">Detection Settings</p>',
        unsafe_allow_html=True
    )
    ...
    selected_model = st.sidebar.selectbox(
        ...
        if config["model"].get("weights", "yolo11s.pt")
        in ["yolo11n.pt", "yolo11s.pt", "yolo11m.pt"]
        else 1,
    ...
    confidence_val = st.sidebar.slider(
        ...
        0.10,
        1.00,
        float(config["model"].get("confidence", 0.40)),
        0.05,
    ...
    # Update the configuration with the selected model settings
...
def update_kpi_row(placeholders, stats: dict):
    """Update KPI values using the latest analytics summary."""

    placeholders["people"].metric(
        "Total People",
        str(stats["total_people"])
    )

    placeholders["vehicles"].metric(
        "Total Vehicles",
        str(stats["total_vehicles"])
    )

    placeholders["dwell"].metric(
        "Avg Dwell Time",
        f"{stats['average_dwell_time_sec']}s"
    )

    placeholders["alerts"].metric(
        "Security Alerts",
...
def render_event_log(placeholder, events_history: list):
    """Render the security event history table."""
...
def main():
    ...
    st.sidebar.caption(
        f"Session started · {datetime.now().strftime('%H:%M:%S')}"
    )
    ...
    with video_col:
        ...
        st.markdown(
            '<p class="section-label">Live Annotated Feed</p>',
            unsafe_allow_html=True
        )
        ...
        video_placeholder.info(
            "Feed will appear here once analytics starts."
        )

    with log_col:

        st.markdown(
            '<p class="section-label">Security Event Log</p>',
            unsafe_allow_html=True
        )
    ...
    # Stop here until the user starts analytics
    if not start_button:
        return
    ...
    if not video_path or not os.path.exists(video_path):
        st.error(
            "Please select or upload a valid video file first."
        )
        return
    ...
    try:
        ...
        # Metrics do not need to refresh on every detection
        KPI_UPDATE_EVERY = max(
            detect_every * 3,
            1
        )
        ...
        while cap.isOpened():
            ...
            # Decide whether this frame should run object detection
            is_detect_frame = (
                frame_index % detect_every == 0
            )
            ...
            if is_detect_frame:
                ...
                events = analytics.analyze(
                    tracked_objects,
                    frame_index
                )


                # Store and display newly detected events
                for evt in events:
                    ...
                    # Only update the event log when a new event appears
                    render_event_log(
                        log_placeholder,
                        events_history
                    )
                ...
                # Draw detections and security zones
                last_annotated = analytics.draw_zones(
                    result.plot()
                )
            ...
            # Save the annotated video frame
            if writer is not None:

                writer.write(
                    last_annotated
                    if is_detect_frame
                    else analytics.draw_zones(frame.copy())
                )


            # Only push a new image to the browser every
            # `display_every` frames — this is the single biggest
            # cost in the loop.
            if (
                last_annotated is not None
                and frame_index % display_every == 0
            ):

                display_frame = last_annotated


                # Resize preview to improve Streamlit performance
        ...
        if total_frames > 0:
            ...
            progress_bar.progress(
                1.0,
                text=f"Processing frame {total_frames}/{total_frames}"
            )
        ...
        # ------------------------------------------------------------------
        st.markdown(
            '<p class="section-label">Analytics Summary &amp; Exports</p>',
            unsafe_allow_html=True
        )
        ...
        # Display main event statistics
        s1, s2, s3 = st.columns(3)

        s1.metric(
            "Loitering Alerts",
            final_summary["loitering_events"]
        )

        s2.metric(
            "Restricted Zone Entries",
            final_summary["restricted_zone_events"]
        )

        s3.metric(
            "Crowding Events",
            final_summary["crowd_events"]
        )
        ...
        with col_a:
            ...
            # Display the complete summary as JSON
            with st.expander(
                "Full summary (JSON)",
                expanded=False
            ):
                ...
                st.json(
                    {
                        "Total People":
                            final_summary["total_people"],

                        "Total Vehicles":
                            final_summary["total_vehicles"],

                        "Average Dwell Time (sec)":
                            final_summary["average_dwell_time_sec"],

                        "Total Alerts":
                            final_summary["total_alerts"],
        ...
        with col_b:
            ...
            # Download security events as CSV
            if events_history:
            ...
            # Inform the user when video export was disabled
            elif not save_video:

                st.caption(
                    "Video export was disabled for this run — "
                    "enable it in the sidebar to download."
                )
        ...
        st.error(
            f"An error occurred while processing the video: {exc}"
        )
