...
.sentinel-header {
    ...
    padding:1rem 1.5rem; margin-bottom:1rem; border-radius:16px;
...
.section-label {
    font-size:.9rem; font-weight:700; color:#000000 !important;
    text-transform:uppercase; letter-spacing:.5px;
    margin-bottom:.4rem; border-left:3px solid #6FA8FF; padding-left:10px;
}
...
section[data-testid="stSidebar"] {
    background:#F5F7FA; border-right:1px solid #D9DDE5;
}
...
def render_header(status="idle"):
    """Display app title and current processing status."""
    status_map = {
...
def resolve_video_source():
    """Get video from sample folder or user upload."""
    ...
    if source == "Select Sample Video":
        ...
        if files:
            ...
            st.sidebar.warning("No sample videos found.")
    else:
...
def resolve_settings(config):
    """Get model and performance settings from sidebar."""
    ...
    confidence = st.sidebar.slider(
        "Confidence threshold", 0.10, 1.00,
        float(config["model"].get("confidence", 0.40)), 0.05
    )
    ...
    config["model"]["weights"] = model_name
...
def render_kpis(container):
    """Create the four main KPI cards."""
...
def update_kpis(kpis, stats):
    """Update KPI values from analytics."""
...
def render_event_log(placeholder, events):
    """Display detected security events."""
    if events:
...
def main():
    ...
    kpis = render_kpis(kpi_container)
    ...
    try:
        ...
        with col2:
            ...
            if output_path and os.path.exists(output_path):
                ...
                st.caption("Video export was disabled for this run.")
